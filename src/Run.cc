//  (Description)
//    FirstTestRun Class is for accumulating scored quantities which is
...
/*
Constructor (args, vector<string> of multifunctional detector names)
- Grabs the SDManager pointer
- Loops through each of the MFDs found in the SDManager
   - Loops through each of the primitive scorers found in the MFD
   - Adds collectionname, collection ID, and 2 run maps (one for dose one for uncertainty) to a vector
   list
*/
Run::Run(const std::vector<G4String> mfdName): G4Run()
{
  ...
  for ( G4int idet = 0; idet < Nmfd ; idet++){  // Loop for all MFD.
    ...
    G4MultiFunctionalDetector* mfd =
      ...
      G4cout << "MFD Detector Name: " << detName << G4endl;
    ...
    if (mfd) 
    {
      ...
      for (G4int icol = 0; icol < mfd->GetNumberOfPrimitives(); icol++)
      {
        ...
        if ( collectionID >= 0 )
        {
          ...
          //extra run map to hold square of the scored quantity per event (for event by event
          //statistics
          theRunMap.push_back(new G4THitsMap<G4double>(detName,collectionName+"_sum of squares"));
        } 
        else 
        {
    ...
    else { G4cout << "MFD: " << detName << " does not exist" << G4endl;}
  }
}
...
//  Joseph:
//  Takes HitsCollection of this event,
//  Loops through collections and maps them to run map
//  Statistical calculation of uncertainty is done here as well
void Run::RecordEvent(const G4Event* aEvent)
{
  ...
  //G4cout << "Number of Collections: " << Ncol << G4endl;
  ...
  {  // Loop over HitsCollection
    ...
    if ( EvtMap )  
    {
      ...
      *theRunMap[2*i] += *EvtMap; //factor 2 so every second runmap holds output of a scorer
      //extra run map (2*i+1) to hold square of the scored quanity per event

      //Get pointer to beginning of event map
      std::map<G4int,G4double*>::iterator iter = EvtMap->GetMap()->begin();
      for(; iter != EvtMap->GetMap()->end(); iter++) 
      {
        //This is how we get the statistical uncertainty by taking the square (I think)
        *(iter->second) = (*(iter->second)) * (*(iter->second));
        // assumes contents of EvtMap are not necessary after being added to runmap
      }
      *theRunMap[2*i+1] += *EvtMap;
      //======================================================
    }
  }
}
...
G4THitsMap<G4double>* Run::GetHitsMap(const G4String& fullName)
{
  ...
  for ( G4int i = 0; i < Ncol; i++)
  {
    //theCollName is linked to 0,2,4 etc runmaps(runmaps1,3,5 are for uncertainties)
    if ( theCollName[i] == fullName )
    {
        return theRunMap[2*i];
    }
  }
    return NULL;
}
...
void Run::DumpAllScorer()
{
  ...
  //i+=2 so does not print the sum of squares maps
  for ( G4int i = 0; i < n ; i+=2 ){
    ...
    if ( RunMap ) {
      ...
      {
         G4cout << "  index: " << itr->first
         //<< "  Run Value : " << G4BestUnit(*(itr->second),"Dose")
         //Gray per unit area(???)
         //The way this is set up right now is that this is outputting the same as runaction, but not 
         //normalized per particle
